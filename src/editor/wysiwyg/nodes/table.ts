        ...
        return () => (state, dispatch) => {
            ...
            if (map) {
                ...
                const cursorPos = createTextSelection(
                    tr.delete(startOffset, tableEndOffset),
                    startOffset
                );
            ...
            if (anchor && head) {
                ...
                const targetColIdx =
                    direction === Direction.LEFT
                        ? selectionInfo.startColIdx
                        : selectionInfo.endColIdx + 1;
                    ...
                    for (let rowIdx = 0; rowIdx < totalRowCount; rowIdx += 1) {
                        for (let colIdx = startColIdx; colIdx <= endColIdx; colIdx += 1) {
                            if (
                                ...
                                !map.extendedColspan(rowIdx, colIdx)
                            ) {
    ...
    private deleteCells(): Command {
        return (state, dispatch) => {
            ...
            if (anchor && head && !textSelection) {
                const map = TableOffsetMap.create(anchor)!;
                const { startRowIdx, startColIdx, endRowIdx, endColIdx } = map.getRectOffsets(
                    anchor,
                    head
                );

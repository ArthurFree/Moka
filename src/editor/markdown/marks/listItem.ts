...
export class ListItem extends Mark {
    ...
    private extendList(): Command {
        return ({ selection, doc, schema, tr }, dispatch) => {
            ...
            const { to, startFromOffset, endFromOffset, endIndex, endToOffset } =
                getRangeInfo(selection);
        ...
        return () =>
            ({ doc, tr, selection }, dispatch) => {
                ...
                for (let line = startLine; line <= endLine; line += 1) {
                    ...
                    const context: ToListContext<MdNode> = {
                        toastMark,
                        mdNode,
                        doc,
                        line,
                    ...
                    const endOffset = this.changeToListPerLine(tr, changedResults, {
                        from: getNodeContentOffsetRange(doc, changedResults[0].line - 1)
                            .startOffset,
                        startLine: changedResults[0].line,
                        endLine: last(changedResults).line,
                        indexDiff: 1
    ...
    private changeToListPerLine(
        ...
        for (let i = startLine - indexDiff; i <= endLine - indexDiff; i += 1) {
            ...
            const [changedResult] = changedResults.filter(
                (result) => result.line - indexDiff === i
            );

...
export class ToastMark implements ToastMarkParser {
    ...
    private parseRange(
        ...
        const isOpenedLastChildCustomBlock =
            lastChild && isCustomBlock(lastChild) && lastChild.open;
    ...
    private parse(startPos: Pos, endPos: Pos, lineDiff = 0): ParseResult {
        ...
        const startLine = startNode
            ? Math.min(startNode.sourcepos![0][0], startPos[0])
            : startPos[0];
